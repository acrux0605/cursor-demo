...
// - https://www.regular-expressions.info/email.html (RFC 5322 공식 패턴)
...
/**
 * 도메인 라벨이 RFC 1035 길이 제한(63자)을 만족하는지 검사한다.
 * IP 리터럴([...])은 정규식에서 처리하므로 여기서는 건너뛴다.
 * @param {string} email
 * @returns {boolean}
 */
function hasValidDomainLabels(email) {
  const domain = email.slice(email.lastIndexOf('@') + 1);
  if (domain.startsWith('[')) return true;

  return domain.split('.').every((label) => label.length <= MAX_DOMAIN_LABEL_LENGTH);
}
...
export function isValidEmail(email) {
  ...
  if (!hasValidDomainLabels(email)) return false;
